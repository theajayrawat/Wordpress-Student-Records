...
//add
function my_form_submission()
{
	if (isset($_POST['name']) && isset($_POST['number']) && isset($_POST['course'])) {
		global $wpdb;
		...
		$result = $wpdb->insert('wp_student', $data_to_insert);

		if ($result) {
			header("location:http://localhost/wordpress/wordpress/student-view/");
		} else {
			echo 'Failed to insert data.';
		}
	} else if (isset($_POST['name']) && isset($_POST['coordinator'])) {
		global $wpdb;
		...
		$result = $wpdb->insert('wp_course', $data_to_insert);
		...
		$result = $wpdb->insert('wp_coordinator', $data_to_insert);
	...
	} else if (isset($_POST['name']) && isset($_POST['course'])) {
		global $wpdb;
		...
		$result = $wpdb->insert('wp_subject', $data_to_insert);
...
add_action('template_redirect', 'my_form_submission');
...
{ ?>
	<script>
		jQuery(document).ready(function($) {
			...
			const size = $('.ajax-button').attr('size');
			for(let i=1;i<=size;i++){
				const id="#ajax-button"+i;
